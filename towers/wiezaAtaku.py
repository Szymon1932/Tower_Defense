...
class WiezaAtaku(Wieza):
    ...
    def atakuj(self, wrogowie):
        ...
        for w in wrogowie:
            ...
            odleglosc = math.sqrt(
                (self.x - w.klatka.get_width() / 2 - x) ** 2 + (self.y - w.klatka.get_height() / 2 - y) ** 2)
            if odleglosc < self.zasieg:
                self.w_zasiegu = True
                wrogowie_tab.append(w)  # dodawanie najblizszego przeciwnika do tablicy do ataku

        wrogowie_tab.sort(key=lambda
            przeciwnik: przeciwnik.x)  # sortowanie po wspolrzednej X przeciwnika. Ostatni element ma najwiekszy X
